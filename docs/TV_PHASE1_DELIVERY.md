...
用户确认测试包缺少凭据时直接提示。最终已撤回下方诊断阶段的自动官方接口回退：
启用镜像且缺少凭据时不发送评论请求，分集页明确提示并禁用刷新。手动关闭镜像仍使用官方接口。
发布前全量 331 项测试通过，另新增提示组件测试；无凭据和测试凭据配置各 4 项测试通过。
发行版本 `2.3.1-tv-preview.5`，ARMv7 versionCode 203121，沿用既有测试签名，可覆盖安装。

### 本地包 Bangumi 评论请求

- 小米电视分集评论页显示加载失败；镜像评论接口缺少签名时返回 401 `invalid request signature`。本地包未注入 `KAZUMI_APPID` / `KAZUMI_KEY`。
- 官方接口 `next.bgm.tv/p1/episodes/1701430/comments` 实测返回 200 与评论列表。没有镜像凭据的构建将三类公开评论请求保留在官方接口，其他请求继续遵循镜像设置；有凭据的构建保持签名镜像行为。
- 无凭据与测试凭据两种配置各 3 项请求路由测试通过，改动文件静态分析无问题；签名测试使用拦截响应，不向服务器发送测试凭据。
- 已安装 `physical-tv-comments-test.apk`（`2.3.1-a-comments-test`，203111），但真机评论尚未恢复：日志确认官方评论请求连接超时 12012ms。电脑经现有代理请求成功，禁用代理直连也在 15 秒超时；电视系统 HTTP 代理为空。剩余为官方接口网络可达性问题，未修改电视或电脑的代理设置。

### 播放线路菜单焦点

- 真机展开线路菜单后，向下键仍能移动被菜单遮挡的剧集焦点。TV 现在在菜单展开后将焦点交给当前线路，并为聚焦选项显示边框。
- 保留上游菜单外观和手机操作方式；上下键切换选项、确认选择、再次打开聚焦已选线路、取消后恢复入口焦点均有回归覆盖。
- 相关 9 项测试通过，改动文件静态分析无问题。真机候选包为 `physical-tv-road-focus-test.apk`（`2.3.1-a-road-focus-test`）。
- 已保留数据覆盖安装到小米电视；实际打开菜单后向下键可聚焦线路 2，截图 `artifacts/phase1-hardening/road-fixed-down.png`。真机反向及确认操作尚未完整验收。
