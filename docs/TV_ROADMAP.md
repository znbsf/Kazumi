...
- `v2.3.0-tv-preview.4` / `203028` 发布同一 AVD / 真机验证 APK，仅重命名文件，内部版本仍为 `2.3.0-ui14-test`。
- 修复 UI-14 首页海报双向自动滚动，避开固定分类栏，取消过期请求，处理卡片回收与侧栏 / 详情返回。
- 发布前 264 项测试通过，含 14 项新增首页专项；正常包已完成 Google TV AVD 与 MiTV / Android 9 首页短程回归。
- 真机确认 55 → 49 → 1、侧栏 / 真实详情返回、快速反向及 ADB 长按；不是实体遥控器全键或本轮视频播放验收。
- 初始焦点疑点、UI-13 进度保存、UI-10 网络错误反馈与 UI-12 渠道文案仍待处理。本包无演示弹幕和在线弹幕凭证。
- [下载 Preview 4](https://github.com/znbsf/Kazumi/releases/tag/v2.3.0-tv-preview.4) · [发行说明](TV_PREVIEW_4.md) · [回归与真机记录](TV_HOME_SCROLL_REGRESSION.md)。
