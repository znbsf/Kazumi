...
- 首页首列回侧栏与选集双描边问题仍未修复，验证码和实体遥控器长返回保留待办。
...
## 发布后 UI 审查（2026-09-06，待实施）

当前公开 APK 仍为 Preview 2 / 203010。这次仅补充文档，不修改应用、重打 APK 或推进发行标签。
完整 UI-01～UI-12 待办及验收要求记录在 [README 的 UI 计划](../README.md#ui-plan)。

- **优先处理 UI-01 / UI-02：**首页首列返回侧栏和播放状态 / 焦点相似描边，已有真机复现。
- **先验证 UI-03 / UI-04：**搜索输入区域禁止后代聚焦，以及历史卡片内部操作可能不可达，
  属于代码审查风险，尚未完成遥控实测；不能登记成已确认故障或已修复。
