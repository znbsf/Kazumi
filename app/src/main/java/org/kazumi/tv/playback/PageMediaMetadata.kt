...
object PageMediaMetadata {
    fun extract(html:String):List<String> = Jsoup.parse(html).select("script:not([src])").take(64).mapNotNull { script ->
        val text=script.data()
        val assignment=Regex("(?:^|[;\\s])(?:var\\s+)?player_[a-zA-Z0-9_]+\\s*=\\s*(?=\\{)").find(text) ?: return@mapNotNull null
        runCatching {
